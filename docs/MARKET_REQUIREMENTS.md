...
EdgeFlow Industrial Controller 面向 Linux 系统软件、工业边缘计算、工业网关、能源 IoT、充电桩控制器和初中级储能 EMS 岗位。真实招聘中常见要求包括：

- 工业设备接入、协议适配和设备状态建模。
- Modbus RTU/TCP、TCP、UDP、MQTT 等通信协议。
- Linux 多线程、epoll、Socket、CLI、systemd 和 watchdog。
- 状态机、告警联锁、指令调度和边缘本地自治。
- Linux/RTOS 多线程、网络编程、现场调试和稳定性优化。
- 断网本地运行、日志、看门狗、长期稳定运行。

本项目只选择三个月内可落地、可验证、能体现 Linux 系统软件和工业控制器能力的子集。BMS/PCS/Meter、分时电价和削峰填谷作为内置示例场景，不代表项目要实现真实商业 EMS。
